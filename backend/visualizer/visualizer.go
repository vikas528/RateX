...
func HandleGetVisualizerState(resw http.ResponseWriter, req *http.Request, server *config.Server) {
	...
	case "token_bucket":
		...
		out["tokens"], out["percent_full"] = getVisualizerStateForTokenBucket(ip, server, context)

	case "fixed_window":
		out["count"], out["ttl_ms"], out["window_ms"], out["percent_used"] = getVisualizerStateForFixedWindow(ip, server, context)

	case "sliding_window":
		out["window_seconds"] = server.GetConfig().WindowSecs

		out["count"], out["request_ages_ms"], out["percent_used"] = getVisualizerStateForSlidingWindow(ip, server, context)
	}

	utils.JsonResponse(resw, http.StatusOK, out)
}

func getVisualizerStateForTokenBucket(ip string, server *config.Server, context context.Context) (token float64, percentFull float64) {
	key := fmt.Sprintf("tb:%s", ip)
	data, _ := server.GetRedisClient().HMGet(context, key, "tokens", "last_refill").Result()
...
func getVisualizerStateForFixedWindow(ip string, server *config.Server, context context.Context) (count int64, ttlMs int64, windowMs int64, percentUsed float64) {
	bucket := time.Now().Truncate(time.Duration(server.GetConfig().WindowSecs) * time.Second).Unix()
	key := fmt.Sprintf("fw:%s:%d", ip, bucket)
	count, _ = server.GetRedisClient().Get(context, key).Int64()
	ttl, _ := server.GetRedisClient().PTTL(context, key).Result()
